...
(defparameter *mwe1* '("cento_e_quatro"
                       "cinquenta_e_três"
                       "setenta_e_cinco"
                       "trinta_e_sete"))
(defparameter *mwe2* '("meia_dúzia"))
...
(defun fix-corpus (sentences)
  (flet ((collection-function (str)
    ...
    (dolist (mwe *mwe1*)
      (mapc (lambda (s) 
              (mapc (lambda (tks) 
                      (fix-mwe tks mwe (sentence-meta-value s "sent_id") :posmwe "NUM" :deprel "flat" :composition '("NUM" "CCONJ" "NUM")))
                    (funcall (collection-function mwe) (sentence-tokens s)))) sentences))

    (dolist (mwe *mwe2*)
      (mapc (lambda (s) 
              (mapc (lambda (tks) 
                      (fix-mwe tks mwe (sentence-meta-value s "sent_id") :posmwe "NUM" :deprel "flat" :composition '("NUM" "NUM")))
                    (funcall (collection-function mwe) (sentence-tokens s)))) sentences)))
  sentences)

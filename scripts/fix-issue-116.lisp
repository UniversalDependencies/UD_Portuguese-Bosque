...
(defparameter *mwe1* '("bem_estar"))
(defparameter *mwe2* '("máquinas_de_escrever" "modo_de_falar"))
(defparameter *mwe3* '("instituto_de_reincerção_social"))
...
(defun fix-corpus (sentences)
  (flet ((collection-function (str)
    ...
    (dolist (mwe *mwe1*)
      (mapc (lambda (s) 
              (mapc (lambda (tks) 
                      (fix-mwe tks mwe (sentence-meta-value s "sent_id") :posmwe "NOUN" :deprel "compound" :composition '("NOUN" "VERB")))
                    (funcall (collection-function mwe) (sentence-tokens s)))) sentences))

    (dolist (mwe *mwe2*)
      (mapc (lambda (s) 
              (mapc (lambda (tks) 
                      (fix-mwe tks mwe (sentence-meta-value s "sent_id") :posmwe "NOUN" :deprel "compound" :composition '("NOUN" "ADP" "VERB")))
                    (funcall (collection-function mwe) (sentence-tokens s)))) sentences))

    (dolist (mwe *mwe3*)
      (mapc (lambda (s) 
              (mapc (lambda (tks) 
                      (fix-mwe tks mwe (sentence-meta-value s "sent_id") :posmwe "NOUN" :deprel "compound" :composition '("NOUN" "ADP" "NOUN" "ADJ")))
                    (funcall (collection-function mwe) (sentence-tokens s)))) sentences)))
  sentences)

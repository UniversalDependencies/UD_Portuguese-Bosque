;; To run:
;; - verify if files old and new are at right place
;; - increase memory
;; (in sbcl, run sbcl as "sbcl --dynamic-space-size N", replacing N with a number. It works with N = 6000)
...
(defparameter new (cl-conllu:read-conllu "bosque.udep.conll"))

(defun generate-distance-matrix ()
...
(defun execute ()
  (let ((distance-matrix (cl-store:restore "data.out")))
    (print "Data restored")
    (let ((men (transform-to-mens-preferences distance-matrix)))
      (print "Men ready")
      (let ((women (transform-to-womens-preferences distance-matrix)))
	(print "Woman ready")
	(let ((result (stable-matching men women)))
	  (print "Matching done!")
	  (cl-store:store result #P"matching.out"))))))
